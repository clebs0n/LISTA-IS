...
vector<vector<pair<int, float>>> numbers; vector<vector<pair<int, float>>> source; vector<float> multVector; //numbers = matriz esparsa; source = segunda matriz esparsa ; multvector = vetor da primeiram operação 
float finalResult[100]; //resultado multiplicação matriz esparsa pelo vetor denso
float finalRes[100][100]; // matriz resultante da multiplicação de matriz esparsa por matriz esparsa ou matriz densa
int maxRow; //variavel que armazena a linha da matriz esparsa usada em todas as operações       
int maxColumn;  // variavel que armazena a coluna da matriz esparsa usada em todas as operações
int numThread;  // número de threads digitadas pelo usuário.. O número pode ser maior que o nº de colunas da matriz, isso não altera o resultado final(uma vez que o loop só roda se o id da thread for menor que o tamanho da matriz)

void * matrixVectorMult(void * args){

    int tId = *((int*) args); // casting do argumento recebido, que nesse caso, é o threadId
    for (int i = tId; i < numbers.size(); i+=numThread) { // Threads com id's ímpares executarão linhas ímpares, e threads com id's pares, linhas pares. Por esse motivo, não são necessários mutexes
